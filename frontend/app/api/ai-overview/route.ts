...
// Lazy (added 5/5/26): GROQ_API_KEY is optional so CI builds without AI
// keys don't fail at module load. The AI overview route returns 503 at
// request time when the key is unset.
...
export async function GET(req: NextRequest) {
  ...
  if (!groq) {
    return NextResponse.json(
      { error: "AI overview disabled: GROQ_API_KEY not configured" },
      { status: 503 }
    );
  }
