...
export default function CourseDetailPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      const courseData = await cachedFetch<Course>(courseKey, COURSE_TTL, async () => {
      ...
      const reviewsKey = `reviews:${id}:page:1`;

      const cachedReviews = getCached<Review[]>(reviewsKey);
      if (cachedReviews) setReviews(cachedReviews);

      const reviewData = await cachedFetch<Review[]>(reviewsKey, REVIEWS_TTL, async () => {
